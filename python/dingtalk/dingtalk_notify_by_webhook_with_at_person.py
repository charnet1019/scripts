...
def get_message(content, user_list):
    message = {
        "msgtype": "text",
        "text": { 
            "content": content
...
def send_ding_message(webhook, secret, content, user_list):
    ...
    # 请求数据
    message = get_message(content, user_list)
    # 对请求的数据进行json封装
...
if __name__ == "__main__":
    # 运维团队机器人
    devops_robot_webhook ="https://oapi.dingtalk.com/robot/send?access_token=0c8e2f23e73fca99043a34be8c43d76dxxxxxxxxxxxxxx"
    devops_robot_secret="SEC767ff0761242961b5be36e26b7525f0ff7261bexxxxxxxxxxxxxxxxxxxx"
    ...
    if notify_type == 'devops':
        send_ding_message(devops_robot_webhook, devops_robot_secret, content, user_list)
    else:
        print("不支持的robot类型")
        sys.exit()
